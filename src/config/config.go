...
func NewConfig() (GlobalConfig, error) {
	...
	if rabbitPass == "" {
		return GlobalConfig{}, fmt.Errorf("RABBITMQ_PASSWORD environment variable is required")
	}
	...
	if dbPass == "" {
		return GlobalConfig{}, fmt.Errorf("POSTGRES_PASSWORD environment variable is required")
	}
